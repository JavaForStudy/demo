...
public class BeanRefUtil {
    ...
    public static Logger log = LoggerFactory.getLogger(BeanRefUtil.class);
    ...
    static {
        ...
        conversionService.addConverter(new BigDecimalToStringConverter());
        conversionService.addConverter(new BigIntegerToStringConverter());
        conversionService.addConverter(new ByteToBooleanConverter());
    }
    ...
    public static <T> T copyBean(Object source, Class<T> destinationClass, Boolean ignoreZero) {
        Map<String, String> valMap = beanToMap(source);
        T t = null;
        try {
    ...
    public static <T> T mapToBean(Map<String,Object> map, Class<T> clzz){
        try {
            // 有下划线就转为驼峰
            Set<String> lineKeys = map.keySet().stream()
                    .filter(it -> it.contains("_")).collect(Collectors.toSet());
            for (String lineKey : lineKeys) {
                map.put(StringUtil.lineToHump(lineKey),map.get(lineKey));
            }

            // 封装数据
            T e = clzz.newInstance();
            BeanWrapper beanWrapper = PropertyAccessorFactory.forBeanPropertyAccess(e);
            beanWrapper.setConversionService(conversionService);
            beanWrapper.setPropertyValues(new MutablePropertyValues(map),
                    true, false);

            return e;
        } catch (Exception e) {
            log.error("以默认构造函数创建实例失败，请检查是否有公开的默认构造方法", clzz);
            throw new BusinessException(e.getMessage());
        }
    }
    public static <T> List<T> mapListToBeanList(List<Map<String,Object>> mapList, Class<T> clzz){
